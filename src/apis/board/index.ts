import { PostCommentRequestDto, PutBoardRequestDto } from './dto/request/index';
...
export const postBoardRequest = async (requestBody: PostBoardRequestDto, accessToken: string) => {
    const result = await axios.post(POST_BOARD_REQUEST_URL, requestBody, bearerAuthorization(accessToken))
        .then(requestHandler<ResponseDto>) // 성공했을시
        .catch(requestErrorHandler); // 실패했을시
    return result;
};
...
// function:A Q& 전체 리스트 불러오기 API 함수
